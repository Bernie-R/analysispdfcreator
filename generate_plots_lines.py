...
def create_plot_acrylates(dir):
    ...
    for row in range(3,df.shape[0]):
        ...
        plt.plot(x1, y1, marker = 'o', ls = "--", c = "k")
        plt.plot(x2, y2, marker = 'o', ls = "--", c = "k")
        ...
        b4, =ax.plot([], markersize=15, linestyle="--", color="k",  label="Migration Limit")
        if failed_migration == True:
            ...
            ax.legend(handles=[b4, b2, b3])
        else:
            #ax.legend(handles=[b1, b2])
            ax.legend(handles=[b4, b2])
